...
The information below is referenced from the [BRFSS User Guide](http://www.cdc.gov/brfss/data_documentation/pdf/userguidejune2013.pdf)
States are responsible to execute the interviewing process according to BRFSS protocols.  Surveys are conducted using the following procedure:
...
We learned above BRFSS attempts to use random sampling techniques to remove bias from the surveys.  If true, then the results of the surveys should be gernerailzable to the populations.  If generalizable, results suggest causation.  Causation can only be inferred from a randomized experiment.

For a study to be defined as an experiemnt the four principles of experimental design must be fulfilled:
...
It is clear from the BRFSS Data Collection provided above that the interviewing process is intended to be a random process.  BRFSS has gone to resonable lengths to reduce bias.  However, often bias cannot be completed elimiated.  For example, in surveys, practicioners must guard against non-response and convenience sampling.

Non-response bias occurs when a low percentage of the randomly sampled respondensts respond, they may not represent the viiews of the population.  Similarly, easily accessed people may provide data inconsistent with the larger population.  Both of these bias types could exist in the BRFSS data - there is no information refuting this possibility.

Also, the number of responses may not be represenrative.  

```{r recordCounts}
byState <- modData %>% group_by(X_state) %>% summarize(count_records = n()) %>% arrange(desc(count_records))
...
Let's first get the data.  Duke made it pretty easy - it is really an RData file so we can just load it into the project directly.  I first set my working directory and then load the data.  Let's also take the time to load the required packages for this project.
...
- Many variables, such as age, race, education, as well as variables that measure counts of events (drinks, times eating fruit, etc.) have alternate versions in the Calculated Variables section of the data set.

Let's get only the data we need.  I know I want to look at data between the US States and the US Territories.  I'll use this information in my data anlysis later.
...
```
Let us now create a new column so we can easily identify US States from Puerto Rico and Guam.  I will assign the value 1 to US States,  2 to Puerto Rico and 3 to Guam:
```{r labelStates}
...
```
Usually a safe maximum NA threshold is 5% of the records for large data sets. If missing data for a certain feature or sample is more than 5% then you probably should leave that feature or sample out. I will need to violate this rule below because some variables must be used even though many NAs exist. For now, let's see a summary of the NA count in the data by visualizing the NA count:
...
plot5
```
The plot was intentionally designed not to display the variables on the y axis - there are too many of them.  The plot does tell me what I want to know - many of the variables have very high numbers of NAs. We need to remove these variables. I'll use the function `na.omit()` when collecting plot data below.
...
cntUS_Males_int <- nrow(tmpdata1 %>%  filter(sex=="Male", internet=="Yes"))
cntUS_Males_no_int <- nrow(tmpdata1 %>%  filter(sex=="Male", internet=="No"))
cntUS_Females_int <- nrow(tmpdata1 %>%  filter(sex=="Female", internet=="Yes"))
cntUS_Females_no_int <- nrow(tmpdata1 %>%  filter(sex=="Female", internet=="No"))
...
cntPR_Males_int <- nrow(tmpdata12 %>%  filter(sex=="Male", internet=="Yes"))
cntPR_Males_no_int <- nrow(tmpdata12 %>%  filter(sex=="Male", internet=="No"))
cntPR_Females_int <- nrow(tmpdata12 %>%  filter(sex=="Female", internet=="Yes"))
cntPR_Females_no_int <- nrow(tmpdata12 %>%  filter(sex=="Female", internet=="No"))
...
cntG_Males_int <- nrow(tmpdata13 %>%  filter(sex=="Male", internet=="Yes"))
cntG_Males_no_int <- nrow(tmpdata13 %>%  filter(sex=="Male", internet=="No"))
cntG_Females_int <- nrow(tmpdata13 %>%  filter(sex=="Female", internet=="Yes"))
cntG_Females_no_int <- nrow(tmpdata13 %>%  filter(sex=="Female", internet=="No"))
...
ggplot(data=tmpdata13, aes(x=internet, fill=sex)) + geom_bar() + 
  ...
  annotate(geom="text", x=2, y=16, label=paste("Females: ", cntG_Females_no_int))
...
--------- | :---------: | :--------------: | :------------: | :-----------------: | 
| US | `r cntUS_Males_Total` | `r cntUS_Males_int_percent`  | `r cntUS_Females_Total` | `r cntUS_Females_int_percent` |
...
```{r quest2Plot}
tmpdata1 <- quest2Data %>%  na.omit() %>%  filter(is_State == 1)

cntUS_Males_exer <- nrow(tmpdata1 %>%  filter(sex=="Male", exerany2=="Yes"))
cntUS_Males_exer_no <- nrow(tmpdata1 %>%  filter(sex=="Male", exerany2=="No"))
cntUS_Females_exer <- nrow(tmpdata1 %>%  filter(sex=="Female", exerany2=="Yes"))
cntUS_Females_exer_no <- nrow(tmpdata1 %>%  filter(sex=="Female", exerany2=="No"))
...
#Puerto Rico
tmpdata2 <- quest2Data %>%  na.omit() %>%  filter(is_State == 2)

cntpr_Males_exer <- nrow(tmpdata2 %>%  filter(sex=="Male", exerany2=="Yes"))
cntpr_Males_exer_no <- nrow(tmpdata2 %>%  filter(sex=="Male", exerany2=="No"))
cntpr_Females_exer <- nrow(tmpdata2 %>%  filter(sex=="Female", exerany2=="Yes"))
cntpr_Females_exer_no <- nrow(tmpdata2 %>%  filter(sex=="Female", exerany2=="No"))
...
ggplot(data=tmpdata2, aes(x=exerany2, fill=sex)) + geom_bar() +
...
#Guam
tmpdata3 <- quest2Data %>%  na.omit() %>%  filter(is_State == 3)

cntg_Males_exer <- nrow(tmpdata3 %>%  filter(sex=="Male", exerany2=="Yes"))
cntg_Males_exer_no <- nrow(tmpdata3 %>%  filter(sex=="Male", exerany2=="No"))
cntg_Females_exer <- nrow(tmpdata3 %>%  filter(sex=="Female", exerany2=="Yes"))
cntg_Females_exer_no <- nrow(tmpdata3 %>%  filter(sex=="Female", exerany2=="No"))
...
ggplot(data=tmpdata3, aes(x=exerany2, fill=sex)) + geom_bar() +
...
--------- | :---------: | :--------------: | :------------: | :-----------------: | 
| US | `r cntUS_Males_Total` | `r cntUS_Males_exer_percent`  | `r cntUS_Females_Total` | `r cntUS_Females_exer_percent` |
| Puerto Rico | `r cntpr_Males_Total` | `r cntpr_Males_exer_percent` | `r cntpr_Females_Total` | `r cntpr_Females_exer_percent` |
...
Similar to the fisrt question, the US and Guam seem to follow similar behaviors.
...
We have only the data we need - time to plot!
```{r quest3Plot}
tmpdata <- quest3Data %>%  na.omit() %>% filter(is_State == 1)

cntUS_Exer_Ment <- nrow(tmpdata %>%  filter(exerany2=="Yes", menthlth_YN == "Yes"))
cntUS_Exer_Ment_No <- nrow(tmpdata %>%  filter(exerany2=="Yes", menthlth_YN == "No"))
cntUS_exer_No_Ment <- nrow(tmpdata %>%  filter(exerany2=="No", menthlth_YN == "Yes"))
cntUS_exer_No_Ment_No <- nrow(tmpdata %>%  filter(exerany2=="No", menthlth_YN == "No"))
...
tmpdata2 <- quest3Data %>%  na.omit() %>%  filter(is_State == 2)

cntpr_Exer_Ment <- nrow(tmpdata2 %>%  filter(exerany2=="Yes", menthlth_YN == "Yes"))
cntpr_Exer_Ment_No <- nrow(tmpdata2 %>%  filter(exerany2=="Yes", menthlth_YN == "No"))
cntpr_exer_No_Ment <- nrow(tmpdata2 %>%  filter(exerany2=="No", menthlth_YN == "Yes"))
cntpr_exer_No_Ment_No <- nrow(tmpdata2 %>%  filter(exerany2=="No", menthlth_YN == "No"))
...
tmpdata3 <- quest3Data %>%  na.omit() %>%  filter(is_State == 3)

cntg_Exer_Ment <- nrow(tmpdata3 %>%  filter(exerany2=="Yes", menthlth_YN == "Yes"))
cntg_Exer_Ment_No <- nrow(tmpdata3 %>%  filter(exerany2=="Yes", menthlth_YN == "No"))
cntg_exer_No_Ment <- nrow(tmpdata3 %>%  filter(exerany2=="No", menthlth_YN == "Yes"))
cntg_exer_No_Ment_No <- nrow(tmpdata3 %>%  filter(exerany2=="No", menthlth_YN == "No"))
...
--------- | :---------: | :--------------: | :------------: | :-----------------: | 
| US | `r cntUS_Exer_Total` | `r cntUS_Exer_Ment_percent`  | `r cntUS_Exer_No_Total` | `r cntUS_Exer_No_Ment_percent` |
| Puerto Rico | `r cntpr_Exer_Total` | `r cntpr_Exer_Ment_percent` | `r cntpr_exer_No_Ment` | `r cntpr_Exer_No_Ment_percent` |
