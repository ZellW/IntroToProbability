...
Duke made loading the data easy - it is really an RData file so we can just load it into the project directly.  I first set my working directory and then load the data.  Let's also take the time to load the required packages for this project.
...
It is clear from the BRFSS Data Collection provided above that the interviewing process is intended to be a random process.  BRFSS has gone to reasonable lengths to reduce bias.  However, often bias cannot be completed eliminated.  For example, in surveys, practitioners must guard against non-response and convenience sampling.
...
**Conclusion**:  While BRFSS has taken the step to establish causation through randomization, I do not have confidence all the bias has been removed nor do I believe that sufficiently large samples (or repetition) is available to make conclusions from the data.  At best, associations might be drawn based on data analysis.  

Let's get only the data we need.  I know I want to look at data between the US States and the US Territories.  I'll use this information in my data analysis later.

First we'll check the state data to make sure it is clean and tidy.
```{r checkStates}
...
```{r quest1Plot1}
#US Data
tmpdataUS <- quest1Data %>% na.omit() %>% filter(Heavy_Drinker == "Yes", is_State == 1)

cntUS_Males_int <- nrow(tmpdataUS %>%  filter(sex=="Male", internet=="Yes"))
cntUS_Males_no_int <- nrow(tmpdataUS %>%  filter(sex=="Male", internet=="No"))
cntUS_Females_int <- nrow(tmpdataUS %>%  filter(sex=="Female", internet=="Yes"))
cntUS_Females_no_int <- nrow(tmpdataUS %>%  filter(sex=="Female", internet=="No"))
...
#Puerto Rico Data
tmpdataPR <- quest1Data %>% na.omit() %>% group_by(internet) %>% 
  filter(Heavy_Drinker == "Yes", is_State ==2)

cntPR_Males_int <- nrow(tmpdataPR %>%  filter(sex=="Male", internet=="Yes"))
cntPR_Males_no_int <- nrow(tmpdataPR %>%  filter(sex=="Male", internet=="No"))
cntPR_Females_int <- nrow(tmpdataPR %>%  filter(sex=="Female", internet=="Yes"))
cntPR_Females_no_int <- nrow(tmpdataPR %>%  filter(sex=="Female", internet=="No"))
...
#Guam Data
tmpdataG <- quest1Data %>% na.omit() %>% group_by(internet) %>% 
  filter(Heavy_Drinker == "Yes", is_State ==3)

cntG_Males_int <- nrow(tmpdataG %>%  filter(sex=="Male", internet=="Yes"))
cntG_Males_no_int <- nrow(tmpdataG %>%  filter(sex=="Male", internet=="No"))
cntG_Females_int <- nrow(tmpdataG %>%  filter(sex=="Female", internet=="Yes"))
cntG_Females_no_int <- nrow(tmpdataG %>%  filter(sex=="Female", internet=="No"))
...
P1 <- ggplot(data=tmpdataUS, aes(x=internet, fill=sex)) + geom_bar() + 
...
P2 <- ggplot(data=tmpdataPR, aes(x=internet, fill=sex)) + geom_bar() + 
  labs(x = "Internet Use", y = "Count of Survey Respondents") +
...
P3 <- ggplot(data=tmpdataG, aes(x=internet, fill=sex)) + geom_bar() + 
  labs(x = "Internet Use", y = "Count of Survey Respondents") +
...
```{r quest2Plot}
#US Data
tmpdataUS <- quest2Data %>%  na.omit() %>%  filter(is_State == 1)

cntUS_Males_exer <- nrow(tmpdataUS %>%  filter(sex=="Male", exerany2=="Yes"))
cntUS_Males_exer_no <- nrow(tmpdataUS %>%  filter(sex=="Male", exerany2=="No"))
cntUS_Females_exer <- nrow(tmpdataUS %>%  filter(sex=="Female", exerany2=="Yes"))
cntUS_Females_exer_no <- nrow(tmpdataUS %>%  filter(sex=="Female", exerany2=="No"))
...
#Puerto Rico Data
tmpdataPR <- quest2Data %>%  na.omit() %>%  filter(is_State == 2)

cntpr_Males_exer <- nrow(tmpdataPR %>%  filter(sex=="Male", exerany2=="Yes"))
cntpr_Males_exer_no <- nrow(tmpdataPR %>%  filter(sex=="Male", exerany2=="No"))
cntpr_Females_exer <- nrow(tmpdataPR %>%  filter(sex=="Female", exerany2=="Yes"))
cntpr_Females_exer_no <- nrow(tmpdataPR %>%  filter(sex=="Female", exerany2=="No"))
...
#Guam Data
tmpdataG <- quest2Data %>%  na.omit() %>%  filter(is_State == 3)

cntg_Males_exer <- nrow(tmpdataG %>%  filter(sex=="Male", exerany2=="Yes"))
cntg_Males_exer_no <- nrow(tmpdataG %>%  filter(sex=="Male", exerany2=="No"))
cntg_Females_exer <- nrow(tmpdataG %>%  filter(sex=="Female", exerany2=="Yes"))
cntg_Females_exer_no <- nrow(tmpdataG %>%  filter(sex=="Female", exerany2=="No"))
...
P4 <- ggplot(data=tmpdataUS, aes(x=exerany2, fill=sex)) + geom_bar() +
...
P5 <- ggplot(data=tmpdataPR, aes(x=exerany2, fill=sex)) + geom_bar() +
  labs(x = "Exercise", y = "Count of Survey Respondents") +
...
P6 <- ggplot(data=tmpdataG, aes(x=exerany2, fill=sex)) + geom_bar() +
  labs(x = "Exercise", y = "Count of Survey Respondents") +
...
```{r quest3Plot}
#US Data
tmpdataUS <- quest3Data %>%  na.omit() %>% filter(is_State == 1)

cntUS_Exer_Ment <- nrow(tmpdataUS %>%  filter(exerany2=="Yes", menthlth_YN == "Yes"))
cntUS_Exer_Ment_No <- nrow(tmpdataUS %>%  filter(exerany2=="Yes", menthlth_YN == "No"))
cntUS_exer_No_Ment <- nrow(tmpdataUS %>%  filter(exerany2=="No", menthlth_YN == "Yes"))
cntUS_exer_No_Ment_No <- nrow(tmpdataUS %>%  filter(exerany2=="No", menthlth_YN == "No"))
...
#Puerto Rico Data
tmpdataPR <- quest3Data %>%  na.omit() %>%  filter(is_State == 2)

cntpr_Exer_Ment <- nrow(tmpdataPR %>%  filter(exerany2=="Yes", menthlth_YN == "Yes"))
cntpr_Exer_Ment_No <- nrow(tmpdataPR %>%  filter(exerany2=="Yes", menthlth_YN == "No"))
cntpr_exer_No_Ment <- nrow(tmpdataPR %>%  filter(exerany2=="No", menthlth_YN == "Yes"))
cntpr_exer_No_Ment_No <- nrow(tmpdataPR %>%  filter(exerany2=="No", menthlth_YN == "No"))
...
#Guam Data
tmpdataG <- quest3Data %>%  na.omit() %>%  filter(is_State == 3)

cntg_Exer_Ment <- nrow(tmpdataG %>%  filter(exerany2=="Yes", menthlth_YN == "Yes"))
cntg_Exer_Ment_No <- nrow(tmpdataG %>%  filter(exerany2=="Yes", menthlth_YN == "No"))
cntg_exer_No_Ment <- nrow(tmpdataG %>%  filter(exerany2=="No", menthlth_YN == "Yes"))
cntg_exer_No_Ment_No <- nrow(tmpdataG %>%  filter(exerany2=="No", menthlth_YN == "No"))
...
P7 <- ggplot(data=tmpdataUS, aes(x=exerany2, fill=menthlth_YN)) + geom_bar() +
  labs(x = "Exercise", y = "Count of Survey Respondents") +
...
P8 <- ggplot(data=tmpdataPR, aes(x=exerany2, fill=menthlth_YN)) + geom_bar() +
  labs(x = "Exercise", y = "Count of Survey Respondents") +
...
P9 <- ggplot(data=tmpdataG, aes(x=exerany2, fill=menthlth_YN)) + geom_bar() +
  labs(x = "Exercise", y = "Count of Survey Respondents") +
