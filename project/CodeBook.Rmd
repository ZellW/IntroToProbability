...
```
Let us now create a new column so we can easily identify US States from Puerto Rico and Guam.  I will assign the value 1 to US States and the value 0 to Puerto Rico and Guam:
```{r labelStates}
modData <- modData %>% mutate(is_State = 1)#label all records as a state
modData$is_State[modData$X_state=="Guam"] <-0
modData$is_State[modData$X_state=="Puerto Rico"] <-0
```
...
names_df <- c("Drink Days Month", "Drinks Month", "Drinks in Day", "Heavy Drinking")
counts_df <- c(AlcoholDays_Past_30_yes, Alcohol_Past_30_yes, Computed_Drinks_Day_yes, Heavy_Alcohol_Calc_yes)
quest1_df <- data.frame(names_df, counts_df)
